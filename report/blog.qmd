...
## 1. The data
I used two main datasets for this, one tracking Olympic medal outcomes going back a century, and one with GDP and population figures for the countries involved. 
...
I needed two indicators for each country and year: GDP per capita and population size. Both indicators were fetched from the World Bank API (https://data.worldbank.org/) using the `wbgapi` package,it returns the data already in a clean format, which saved a lot of manual tidying. 
...
Germany's average of 24.4 medals per Games is the highest in the table desipte having only 11 appearances. That says a lot about how consistently strong West Germany and reunified Germany have been on the rare occasions they competed. Russia (6 Games, 19.7 avg) and East Germany (6 Games, 18.3 avg) show even higher rates, however both stop competing after the Soviet era ended.
:::
...
The scatter plot below plots log GDP per capita against total medals. The upward trend is clear, wealthier countries win more, whether that's down to better facilities, more full time althetes or just more money in the system the pattern is clear.
...
Wealth matters, but there's one thing money can't buy: a home crowd.
...
### 7.1 The model
Once the data was aggregated to country-year levell, I ran an OLS regression to test the main question directly: after controlling for GDP per capita and population size, does hosting the Games have a significant impact on medal success? The model is:
